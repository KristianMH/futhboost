      ...
      let (_,splits, data) =
        loop (j, splits_found, data) =
          (if m==0 then 0 else 2^(m-1), [], data) while j < 2**m do -- maybe keep list of active leafs?
             ...
             --let ha = trace (length points_in_leaf)
             ...
             let (pos_splits) =
               ...
               let gains = map2 (\g h -> gain g h gm hm l1) scan_gis scan_his --|> trace
               let (best_split_idx, max_gain) = arg_max gains
               let ha = best_split_idx
               in
               splits ++ [(sorted_data[best_split_idx], max_gain)] --|> trace
             let ha = trace pos_splits
             let (dim, gain) = arg_max (unzip pos_splits).1
             let value = pos_splits[dim].0
             let node_flag = gain > 0.0
             let data = if gain > 0.0 then do_split data points_in_leaf dim value
                        else data
             in

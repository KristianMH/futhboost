...
let gradient_mse (pred: f32) (orig: f32): f32 = 2*(pred-orig)

let hessian_mse 't (pred: t) (orig: t): f32 =  2.0
...
-- let check_leaf_idx 't (x: ((i32,i32), []t)) (i:i32): bool =
--   x.0 == i


let get_leaf_weight [n] (gis: [n]f32) (his: [n]f32) (l1: f32): f32 =
  let gsum = reduce (+) 0.0 gis
  let hsum = reduce (+) 0.0 his
  in
  -gsum/(hsum + l1)

let gain (gl: f32) (hl:f32) (g:f32) (h:f32) (l1: f32) =
  let gr = g-gl
  let hr = h-hl
  in
  1/2*(gl**2/(hl+l1)+(gr**2/(hr+1))-g**2/(h+l1))
...
let do_split [n][d][b] (orig_data: [n](i32, i32, [d]f32)) (leaf_data: [b](i32, i32, [d]f32))
  ...
  let new_leaf_idx = map2 (\li data -> if data[dim] <= value then li*2 else li*2+1) leaf_idxs data
        ...
        loop (j, splits_found, data) =
          (if m==0 then 0 else 2^(m-1), [], data) while j < 2**m do -- maybe keep list of active leafs?
             ...
             let (pos_splits) =
               ...
               let gains = map2 (\g h -> gain g h gm hm l1) scan_gis scan_his
               let best_split_idx = arg_max gains |> (.0)
               in
               splits ++ [sorted_data[best_split_idx]]
             let (dim, value) = arg_max pos_splits
             let node_flag = value > 0.0
             let data = if value > 0.0 then do_split data points_in_leaf dim value
                        else data
             in
             (j+1, splits_found ++ [(dim, value, node_flag)], data)             
      in

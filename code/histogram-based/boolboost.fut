...
let train_round [n][d][b] (data: [n][d]u16) (bin_bounds: [d][b]f32)
  ...
  let gis = map2 (\p y -> gradient_log p y) preds labels
  let his = map2 (\p y -> hessian_log p y) preds labels
...
let bool_error [n] (labels: [n]f32) (preds: [n]f32) : f32 = -- squared_error labels preds
   map2 (\x y -> if x> 0.5 then 1-y else y) preds labels |> reduce (+) 0|> (/(f32.i64 n))


let train [n][d] (data: [n][d]f32) (labels: [n]f32) (max_depth: i64) (n_rounds: i64)
                       (l2: f32) (eta: f32) (gamma: f32) = --: ([n_rounds]f32, [10]f32) =
  ...
  let (_, error, tree) =
    loop (preds, e, tree) = (inital_preds, results, []) for i < n_rounds do
      ...
      --let train_error = bool_error labels new_preds
  ...
  error
  --unzip4 (res.2)--,
  --(map (\x -> predict x mapped_tree) data[:10], labels[:10], error)

...
-- (data_f: [n][d]f32) 
let train_round [n][d][b] (data_b: [n][d]u16) (bin_bounds: [d][b]binboundaries)
        ...
        while (i <= max_depth) && !(null leafs) do
          ...
          let (new_leafs, new_tree, new_data, new_gis, new_his) = 
            if i == max_depth then
              ...
              let final_tree = scatter tree (map (\t -> t-1) active_leafs) entries
              in
                ([], final_tree, [], [], [])
                --([], tree_full, [], [], [], [])
            else
              ...
              -- heavy hitter in mem. change to u8 and just in time convert?

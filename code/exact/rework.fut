...
let gain (gl: f32) (hl: f32) (g:f32) (h: f32) (l2: f32) (gamma: f32)
  ...
  --let left_missing = (gl+missing_gis_sum)**2/(hl+missing_his_sum+l2)
  --let right_missing = (gr+missing_gis_sum)**2/(hr+missing_his_sum+l2)
  --let left = -1/2f32*(left_missing + gr**2/(hr+l2) + cost_no_split) - gamma
  --let right = -1/2f32*(gl**2/(hl+l2) + right_missing + cost_no_split) -gamma
            ...
            if i == max_depth then -- last level of tree i.e. all nodes should be leafs
              ...
              let (node, new_conds) =
                if best_gain >= 0f32 then
                  ...
                  in
                    (node, new_conds)
                else -- no split, so the node is a leaf
                  --let weight = get_leaf_weight gis_in_node his_in_node l2 eta
                  let weight = eta*(-g_node/(h_node+l2))
                  let leaf = (0i64, weight, false, false)
                  in

...
let train_round [n][d] (data: [n][d]f32) (labels: [n]f32) (preds: [n]f32) (max_depth: i32) 
  ...
  let data_x = zip3 (replicate n 1) p_idxs data
  let active_leafs = [1]
  let (_, res, _, _) =
    ...
    while !(null active_leafs) && (log2 (leaf_count) <= max_depth) do
      ...
      if length points_in_leaf == 1 then -- cannot split node with one ele :) -- min size?
        ...
        let pos_splits = map (\i -> search_splits data_points point_idx
                                                  gis his (i32.i64 i) l2 gamma) (iota d)
        ...
        let (value, data, new_leafs) =
          if node_flag then
            let new_data = do_split data leaf_idx points_in_leaf (i32.i64 dim) value missing_flag

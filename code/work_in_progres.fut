...
let gain (gl: f32) (hl:f32) (g:f32) (h:f32) (l2: f32) (gamma: f32)
  ...
  let left_gain_missing = (gl+missing_gis_sum)**2/(missing_his_sum+hl+l2)
                          + gr**2/(hr+l2) - cost_node
  let right_gain_missing = gl**2/(hl+l2)+ (gr+missing_gis_sum)**2/(missing_his_sum+hr+l2)
                           - cost_node
  in
...
let search_splits [m] (data_dim: [m]f32) (gis: [m]f32) (his: [m]f32)
  ...
  let xgboost_split_val = if best_split_idx < (length sorted_idx)-1 then
                            ...
                            sorted_data[best_split_idx]*2
                            -- apprently multiply with 2 in xgboost impl
  in
...
let do_split [n][d][m] (orig_data: [n](i32, i32, [d]f32)) (li: i32) (point_idxs: [m]i32)
  ...
  let new_leaf_idx = map (\x ->
                            -- checks if val < splitval or missing_direction
...
let train_round [n][d] (data: [n][d]f32) (labels: [n]f32) (preds: [n]f32) (max_depth: i32) 
  ...
  let data_zip= zip3 (replicate n 1) (iota n) data  -- (leaf_id, index to data, data)
  let active_leafs = [1] -- remember off by one in scatter update to tree
  let (_, tree, _) =
    loop (active_leafs, tree, data, l) = (active_leafs, tree, data_zip, 0)
    while !(null active_leafs) && l < max_depth do
      let haha =
        map (\leaf_idx ->
               let (_, point_idxs, data_points) = filter (\x -> x.0 == leaf_idx) data |> unzip3
               in
               if length point_idxs == 1 then
                   let point_idx = (head point_idxs)
                   let weight = eta*(-gis[point_idx]/(his[point_idx]+l2)) --+ min_weight
                   in
                   ((0, weight, false, false), [], [-1i32, -1])
                 else
                   ...
                   in
                   if node_flag then
                     let ndata = do_split data leaf_idx point_idxs data_points
                                          split_dim value missing_flag :> [](i32, i32, [d]f32)
                     let new_leafs = getChildren leaf_idx
                     in
                     ((split_dim, value, node_flag, missing_flag), ndata, new_leafs)
                   else
                     let weight = get_leaf_weight gis his l2 eta
                     in
                     ((0, weight, false, false), [], [-1i32, -1i32])
                     
            ) active_leafs :> []((i32, f32, bool, bool), [](i32, f32, [d]f32), []i32)
      let (new_nodes, ndata, new_leafs) = unzip3 haha
    let new_leafs = flatten new_leafs
    let new_tree = scatter tree (map (\x -> x-1) active_leafs) new_nodes
    in
    (new_leafs, new_tree, data, l+1)
  in
  tree
...
let train [n][d] (data: [n][d]f32) (labels: [n]f32) (max_depth: i32) (n_rounds: i32)
  ...
  let res =
    loop (data, labels, preds, e) = (data, labels, inital_preds, 0) for i < n_rounds do
      ...
      let train_error = error labels new_preds
...
let eval = train data[:,:2] data[:,2] 3 2 0.5 0.3 0

...
--do_split data leaf_idx point_idx data_points dim value missing_flag
let do_split [m][n][d] (li_idxs: [n]i32) (li: i32) (point_idxs: [m]i32) (points: [m][d]f32)
                       (dim: i32) (value: f32) (missing_flag: bool): [n]i32 =
  let new_leaf_idxs = map (\x ->
                           if x[dim] < value || (f32.nan == x[dim] && missing_flag) then
                             li*2
                           else
                             li*2+1) points
  in
  scatter (copy li_idxs) point_idxs new_leaf_idxs
...
let do_split_v2 [n][d][m] (orig_data: [n](i32, i32, [d]f32)) (li: i32) (point_idxs: [m]i32)
        ...
        if length point_idxs == 1 then -- cannot split node with one ele :) -- min size?
          ...
          let (value, data, new_leafs) =
            if node_flag then
              ...
              let ndata = do_split_v2 data leaf_idx point_idxs data_points split_dim value missing_flag

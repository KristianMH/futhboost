...
let partition_lifted [n][l][d] 't (conds: [l](i32, t)) (op: t -> t -> bool) (shp: [l]i32)
  ...
  let cond_offsets = scan (+) 0 flag_arr |> map (\t -> t-1) --|> trace
  let cs = map2 (\v i -> let (dim, cond_val) = conds[i]
                         in !(op v[dim] cond_val)) vals cond_offsets 
  ...
  let num_true_segs = segmented_reduce (+) 0 bool_flag_arr true_ints l
  let num_false_segs = map2 (-) shp num_true_segs
  ...
  let true_val_offsets = map2 (\x i -> x + num_false_segs_idxs[i] +seg_offsets[i])
                              true_offsets seg_offsets_idx --|> trace 
  let false_val_offsets = map2 (\x i -> x + seg_offsets[i])
                               false_offsets seg_offsets_idx --|> trace
...
let shp = [2i32,3]
let a = [[1,3],[3,2],[5,43],[83,2],[3,3]] :>[][]i32
let conds = [(1,3),(0,4)]:> [](i32,i32)

let partition_lifted_test1 (dims: []i32) (conds: []i32) (shp: []i32)
                                  (vals: [][]i32) =
  (partition_lifted (zip dims conds) (<) shp vals)
let ha = partition_lifted_test1 [1,0] [3,3] [2,3] [[1,10], [0, 1], [3, 5], [100, 5], [-3, -4]]
